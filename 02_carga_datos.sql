...
--
-- Ejecucion recomendada (desde la raiz del repositorio, para que la
-- ruta relativa de \copy resuelva correctamente):
--   psql -h localhost -p 5432 -U admin -d netflix_db -f sql/02_carga_datos.sql
-- =====================================================================
...
-- ---------------------------------------------------------------------
TRUNCATE TABLE netflix_staging RESTART IDENTITY;

\copy netflix_staging(show_id, type, title, director, cast_members, country, date_added, release_year, rating, duration, listed_in, description) FROM 'data/netflix_titles.csv' WITH (FORMAT csv, HEADER true, ENCODING 'UTF8', QUOTE '"')

-- Conteo de control: filas leidas desde el CSV
-- (debe coincidir con el total de lineas de datos del archivo fuente)
SELECT COUNT(*) AS filas_cargadas_staging FROM netflix_staging;
...
SELECT DISTINCT TRIM(p.person_item)
FROM (
    SELECT unnest(string_to_array(director, ',')) AS person_item
    FROM netflix_staging WHERE director IS NOT NULL
    UNION ALL
    SELECT unnest(string_to_array(cast_members, ',')) AS person_item
    FROM netflix_staging WHERE cast_members IS NOT NULL
) p
WHERE TRIM(p.person_item) <> ''
...
JOIN people p ON p.full_name = TRIM(d.person_item)
...
JOIN people p ON p.full_name = TRIM(a.person_item)
...
-- 1) Coherencia cuantitativa: filas del CSV vs. titulos almacenados
SELECT
    (SELECT COUNT(*) FROM netflix_staging) AS filas_csv,
    (SELECT COUNT(*) FROM titles)          AS titulos_cargados;

-- 2) Registros huerfanos en tablas asociativas (deben ser 0 en todos los casos)
SELECT 'title_countries -> titles'  AS chequeo, COUNT(*) AS huerfanos
FROM title_countries tc LEFT JOIN titles t ON t.title_id = tc.title_id
WHERE t.title_id IS NULL
UNION ALL
SELECT 'title_countries -> countries', COUNT(*)
FROM title_countries tc LEFT JOIN countries c ON c.country_id = tc.country_id
WHERE c.country_id IS NULL
UNION ALL
SELECT 'title_genres -> titles', COUNT(*)
FROM title_genres tg LEFT JOIN titles t ON t.title_id = tg.title_id
WHERE t.title_id IS NULL
UNION ALL
SELECT 'title_genres -> genres', COUNT(*)
FROM title_genres tg LEFT JOIN genres g ON g.genre_id = tg.genre_id
WHERE g.genre_id IS NULL
UNION ALL
SELECT 'title_people -> titles', COUNT(*)
FROM title_people tp LEFT JOIN titles t ON t.title_id = tp.title_id
WHERE t.title_id IS NULL
UNION ALL
SELECT 'title_people -> people', COUNT(*)
FROM title_people tp LEFT JOIN people p ON p.person_id = tp.person_id
WHERE p.person_id IS NULL;

-- 3) Titulos sin rating asignado (esperable: filas con rating vacio en el CSV)
SELECT COUNT(*) AS titulos_sin_rating FROM titles WHERE rating_id IS NULL;

-- 4) Resumen de poblacion por tabla
